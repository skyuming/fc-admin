...
It provides four sub-skills:

| Sub-skill | Use when user wants to... |
|---|---|
| `fc-admin-init` | create a NEW project from an empty directory (scaffold starter) |
| `fc-admin-crud` | generate / modify a business module in an existing project (API + page + route + permissions) |
...
```
1. User wants to create a new project from scratch?         → invoke fc-admin-init
2. User wants to generate / modify a business module?       → invoke fc-admin-crud
3. User asks "how do I use X" about a project mechanism?    → invoke fc-admin-arch
4. User asks "is this code OK / review / check / 走查"?     → invoke fc-admin-review
5. Intent ambiguous or spans multiple?                      → ask the user which one
6. Task unrelated to a Vue 3 element-plus admin?            → reply normally, do NOT invoke any sub-skill
```
...
This skill auto-loads on phrases including: "fc-admin", "fc 后台", "用 fc-admin", "fc admin 帮我", "future-community admin", "vue3 element-plus admin", "fc-admin 帮我建项目", "fc-admin init", "fc-admin 新建项目".
