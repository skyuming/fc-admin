  ...
  <div class="w-full h-[50px] bg-gray-800 dark:bg-[var(--el-bg-color-overlay)]">
    <transition name="sidebarLogoFade">
      ...
      <router-link
        ...
        to="/"
      >
        <img v-if="settingsStore.sidebarLogo" :src="logo" style="width: 30px; height: 30px;" class="w-5 h-5" />
        <span class="ml-3 text-white text-sm font-bold" style="font-family: alimamaFont; font-size: 20px; line-height: 20px;">{{title}}</span>
      </router-link>
    </transition>
  </div>
</template>
